...
class ListViewTest(TestCase):
    ...
    def test_displays_only_items_for_that_list(self):
        ...
        Item.objects.create(text='itemey 2', list=correct_list, priority='prioridade baixa')
        Item.objects.create(text='itemey 3', list=correct_list, priority='prioridade alta')

        other_list = List.objects.create()
        Item.objects.create(text='other list item 1', list=other_list)
        Item.objects.create(text='other list item 2', list=other_list, priority='prioridade média')

        response = self.client.get(f'/lists/{correct_list.id}/')

        self.assertContains(response, 'itemey 1')
        self.assertContains(response, 'itemey 2 - prioridade baixa')
        self.assertContains(response, 'itemey 3 - prioridade alta')
